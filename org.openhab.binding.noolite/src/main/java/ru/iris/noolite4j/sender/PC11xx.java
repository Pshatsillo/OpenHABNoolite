...
public class PC11xx {
	...
	public void open() throws LibUsbException {
		...
		LOGGER.debug("Opening device PC11xx");
	 ...
	 */
	public void close() {
		LOGGER.debug("Closing device PC11xx");
		LibUsb.exit(context);
	}
	...
	private void writeToHID(ByteBuffer command) {
		...
		if (ret != LibUsb.SUCCESS) {
			LOGGER.error("Device PC11XX configuration error");
			LibUsb.close(handle);
			if (ret == LibUsb.ERROR_BUSY) {
				LOGGER.error("Device PC11XX bisy");
			}
			return;
		}
		...
		LOGGER.debug("PC11XX содержимое буффера: " + command.get(0) + " " + command.get(1) + " " + command.get(2) + " "
				+ command.get(3) + " " + command.get(4) + " " + command.get(5) + " " + command.get(6) + " "
				+ command.get(7));
