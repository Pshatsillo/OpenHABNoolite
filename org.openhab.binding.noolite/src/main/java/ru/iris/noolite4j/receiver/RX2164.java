...
public class RX2164 {
	...
	public void start() {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				// Для старта бурем такое число, которе точно не может быть в
				// данный момент в TOGL
				int tmpTogl = -10000;
				...
				while (!shutdown) {
					...
					/**
					 * Пауза нужна для того чтобы иметь возможность записать
					 * буффер в устройство (привязка, отвязка) TODO возможно,
					 * есть решение лучше
					 */
					if (!pause) {
					...
					/**
					 * Сравниваем значение TOGL, чтобы понять, что пришла новая
					 * команда
					 */
					togl = buf.get(0) & 63;

					/**
					 * Получена новая команда TOGL может быть 0
					 */
					if (togl != tmpTogl) {
						...
						case BIND:
							...
							/**
							 * Если привязывается датчик, то он передает
							 * дополнительно свой тип
							 */
						...
						case TEMP_HUMI:
							...
							/**
							 * Информация о температуре, типе датчика и
							 * состоянии батареи размазана по 2 байтам
							 */
							...
							/**
							 * В четвертом байте данных хранятся данные о
							 * состоянии аналогового датчика По умолчанию -
							 * unsigned byte (255)
							 */
	...
	/**
	 * Функция, используемая для привязки устройства на определенный канал
	 * RX2164
	 * 
	 * @param channel
	 *            канал, на которое будет привязано устройство
	 */
	...
	/**
	 * Функция, используемая для отвязки устройства с определенного канала
	 * RX2164
	 * 
	 * @param channel
	 *            канал, с которого будет отвязано устройство
	 */
